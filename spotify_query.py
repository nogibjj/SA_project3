...
def most_songs_year():
    ...
    cursor = con.cursor()
    cursor.execute(
        "SELECT year, MAX (songcount) FROM (SELECT year, COUNT(*) songcount FROM spotify_songs GROUP BY year);"
    )
    songs_year = cursor.fetchall()
    max_songs = songs_year[0]
    print(
        f"The most songs released (based on this dataset) was in {max_songs[0]} with {max_songs[1]} songs."
    )
...
def most_popular():
    ...
    cursor = con.cursor()
    cursor.execute(
        "SELECT DISTINCT(song), artist, year FROM spotify_songs ORDER BY popularity DESC LIMIT 10;"
    )
...
def longest_songs():
    ...
    cursor = con.cursor()
    cursor.execute(
        "SELECT DISTINCT(song), artist, duration_ms FROM spotify_songs ORDER BY duration_ms DESC LIMIT 5;"
    )
    most_pop_songs = cursor.fetchall()
    rank_idx = 1
    for song, artist, length in most_pop_songs:
        print(
            f"#{rank_idx}: {song} by: {artist} is {length / 1000} seconds long ({length / (1000*60):.2f} mins)"
        )
        rank_idx += 1
